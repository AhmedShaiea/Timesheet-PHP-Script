...
@section('content')
    ...
    <div class="row form-inline">
        <div class="col-xs-12">
            <div class="form-group loading">
                <span>{{ trans('messages.DivisionList') }}: <a href="{{ url('/division/create') }}" class="btn btn-success">{{ trans('messages.Create') }}</a></span>
                <select class="form-control" onchange="master_showResultOnReloadedPage('division');" style="display:inline-block;" name="master_amountperpage" id="master_amountperpage">
  ...
  @if (!count($divisions))
      <p>{{ trans('messages.Nodivisions') }}</p>
  @else
      <div class="table-responsive" id="master_show_result">
        <table class="table table-bordered table-hover table-striped">
        <tr>  <!-- ['ID'=>'id', 'Name'=>'name', 'Desc'=>'description', 'Status'=>'status', ''=>'']  -->
            @foreach (json_decode(UserHelpers::getConstants('HEADER_DIVISION_ARRAY'), true) as $key => $val )
            <th class="{{ $val }}">
				@if(!empty($key))
					{!! trans("messages.$key") !!}&nbsp;&nbsp;&nbsp;
				@endif
        ...
        <tr>
			<th>
			    ...
			    <a href="{{ url('/division/edit/' . $p->id) }}" class="btn btn-primary">{{ trans('messages.Edit') }}</a>
				<a href="javascript:void(0);" onclick="master_delete(this, {{$p->id}}, 'division');" class="btn btn-danger">{{ trans('messages.Delete') }}</a>
				@endif
			</th>
